pro get_beam_data,beamid,chord,beam;,profiles
    ...
    beam_atom = alc_input.beam_param.beam_atom
    case beam_atom of
    ...
    grid_pos = cyl2cart([alc_input.beam_geom.r_grid,alc_input.beam_geom.phi_grid+alc_input.beam_geom.beam_port_phi,alc_input.beam_geom.z_grid])
    wall_pos = cyl2cart([alc_input.beam_geom.r_wall,alc_input.beam_geom.phi_wall+alc_input.beam_geom.beam_port_phi,alc_input.beam_geom.z_wall])
    ba = alc_input.beam_geom.beam_apertures
...
;	if alc_input.n_e_prof.n_e_coord eq 0 then begin
;	    n_e = interpol(alc_input.n_e_prof.n_e,alc_input.n_e_prof.n_e_r,chord.r) > 0 ;1/cm^3
;	endif else begin
;	    n_e = interpol(alc_input.n_e_prof.n_e,alc_input.n_e_prof.n_e_r,chord.rho) > 0 ;1/cm^3
;	endelse
;	if alc_input.t_e_prof.t_e_coord eq 0 then begin
;	    t_e = interpol(alc_input.t_e_prof.t_e,alc_input.t_e_prof.t_e_r,chord.r)*1000 > 0 ;eV
;	endif else begin
;	    t_e = interpol(alc_input.t_e_prof.t_e,alc_input.t_e_prof.t_e_r,chord.rho)*1000 > 0;eV
;	endelse
;	if alc_input.z_eff_prof.z_eff_coord eq 0 then begin
;	    z_eff = interpol(alc_input.z_eff_prof.z_eff,alc_input.z_eff_prof.z_eff_r,chord.r) > 1
;	endif else begin
;	    z_eff = interpol(alc_input.z_eff_prof.z_eff,alc_input.z_eff_prof.z_eff_r,chord.rho) > 1
;	endelse
...
;	main_ion = alc_input.plasma_param.main_ion
;	case main_ion of
;	    'H': Z_main = 1
;	    'D': Z_main = 1
;	    'He': Z_main = 2
;	endcase
;	impur_table = alc_input.plasma_param.impur_table
